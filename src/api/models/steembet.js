...
export class SteemBet extends BaseDice {
    constructor(){
        super();
    ...
    async bet(req) {
        ...
        let condition = 'roll_under';
        let currency = req.body.Currency.toLowerCase();
        let target = Math.floor(req.body.Chance) + 1;
        let memo = '{"roll_under":'+target+',"referral":"mydicebot","seed":"mydicebot'+Math.random().toString(36).substring(2)+'"}';
        let bet = amount + ' '+ req.body.Currency.toUpperCase();
        let userName = req.session.username;
        let token = req.session.accessToken;
        let steemBet = 'steem-bet';
	    try{
            let ret = await this._transfer(token, userName, steemBet, bet, memo);
            ...
            let release = steem.api.streamOperations(async function (err, op) {
                if (err) {
                    ...
                    if (op[0] === "transfer" && op[1].to === userName) {
                        if (op[1].from === "steem-bet" && op[1].memo.startsWith("[STEEM-BET]")) {
                            tryQueryCount++;
            ...
            let req = request.post(options,function (e, r, body) {
                if(e) {
                    ...
                    if(body) {
                        ...
                        for(let k  in res) {
                            let tran = res[k][1].op;
                            if (tran[0] == "transfer" && tran[1].from == "steem-bet" && tran[1].memo.startsWith("[STEEM-BET]")) {
